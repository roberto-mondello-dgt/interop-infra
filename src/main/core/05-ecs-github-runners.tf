...
resource "aws_iam_role" "github_runner_task_exec" {
  name = format("%s-github-runner-ecs-task-exec-%s", var.short_name, var.env)
...
resource "aws_iam_role" "github_runner_task" {
  name = format("%s-github-runner-task-%s", var.short_name, var.env)
  ...
  container_definitions = jsonencode([
    {
      ...
      memory    = 4096
      essential = true
      image     = "ghcr.io/pagopa/interop-qa-runner:v1.2.0"
