...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hi:o:v:p",["input=","output=","api-version=", "use-service-prefix"])
    except getopt.GetoptError:
        print('openapi_integration.py -i <input-file> [-o <output-file>] [-v <api-version>] [-p]')
        sys.exit(2)
    ...
    if input_file == '':
        print('openapi_integration.py -i <inputfile> [-o <outputfile>] [-v <api-version>] [-p]')
        sys.exit(2)
