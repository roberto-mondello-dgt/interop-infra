...
variable "ses_identity_arn" {
  description = "ARN of the SES Identity to be used by the IAM user to send emails"
  type        = string
}

variable "ses_configuration_set_arn" {
  description = "ARN of the SES Configuration set to be used by the IAM user to send emails"
  type        = string
}

variable "allowed_recipients_literal" {
  description = "List of recipients to which the IAM user is allowd to send emails. It must contain the exact literals that make each single recipient (e.g. example@pagopa.it)"
  type        = list(string)
  default     = null
}

variable "allowed_recipients_regex" {
  description = "List of recipients to which the IAM user is allowd to send emails. It can contain regex with wildcards (e.g. *@pagopa.it)"
  type        = list(string)
  default     = null
}
