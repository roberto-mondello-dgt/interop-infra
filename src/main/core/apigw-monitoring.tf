...
module "interop_selfcare_0dot0_apigw_monitoring" {
  ...
  env           = var.env
  apigw_name    = module.interop_selfcare_0dot0_apigw[0].apigw_name
  sns_topic_arn = aws_sns_topic.platform_alarms.arn
...
module "interop_selfcare_1dot0_apigw_monitoring" {
  ...
  env           = var.env
  apigw_name    = module.interop_selfcare_1dot0_apigw[0].apigw_name
  sns_topic_arn = aws_sns_topic.platform_alarms.arn
...
module "interop_frontend_assets_apigw_monitoring" {
  ...
  env           = var.env
  apigw_name    = module.interop_frontend_assets_apigw[0].apigw_name
  sns_topic_arn = aws_sns_topic.platform_alarms.arn
