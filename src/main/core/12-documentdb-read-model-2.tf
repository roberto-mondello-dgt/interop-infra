# TODO rename file after migration

# TODO: rename after migration
data "aws_subnets" "read_model_v2" {
  filter {
    name   = "vpc-id"
...
# TODO remove comment after deleting other docdb file
...
resource "aws_docdb_subnet_group" "read_model_v2" {
  ...
  subnet_ids = data.aws_subnets.read_model_v2.ids
}
...
resource "aws_docdb_cluster" "read_model_v2" {
  ...
  final_snapshot_identifier = format("read-model-%s", random_id.read_model_snapshot_id.hex)
